...
class TipoPago extends Component
{
    ...
    {
        if ($this->readyToLoad) {
            $datas =  CtgTipoPago::where('name', 'ilike', '%' . $this->search . '%')
                ->orderBy('id', 'DESC')->paginate(10);;
        } else {
            $datas = [];
        }
        return view('livewire.catalogos.tipo-pago', compact('datas'));
    }
    ...
    public function save()
    {
        ...
        try {
            $this->validate();
            CtgTipoPago::create($this->only(['name']));
            DB::commit();
            $this->reset('name');
    ...
    #[Validate('required')]
    public $name;
    public function setData(CtgTipoPago $data)
    {
        $this->limpiar();
        $this->name = $data->name;
    ...
    public function update()
    {
        ...
        try {
            ...
            $data = CtgTipoPago::find($this->data_id);
            $data->update([
    ...
    #[On('delete-data')]
    public function delete(CtgTipoPago $data)
    {
        try {
            $data->delete();
    ...
    #[On('baja-data')]
    public function baja(CtgTipoPago $data)
    {

        try {
            $data->update([
                'status_ctg_tipo_pago' => 0,
            ]);
            $this->dispatch('success-data', "El registro se dio de baja correctamente.");
        } catch (\Exception $e) {
            $this->dispatch('error', "No se pudo completar la accion, intente mas tarde.");
    ...
    #[On('delete-reactivar')]
    public function reactivar(CtgTipoPago $data)
    {
        try {
            $data->update([
                'status_ctg_tipo_pago' => 1,
            ]);
