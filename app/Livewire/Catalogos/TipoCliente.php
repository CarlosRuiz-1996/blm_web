...
class TipoCliente extends Component
{
    ...
    {
        if ($this->readyToLoad) {
            $datas =  Ctg_Tipo_Cliente::where('name', 'ilike', '%' . $this->search . '%')
                ->orderBy('id', 'DESC')->paginate(10);;
        } else {
            $datas = [];
        }
        return view('livewire.catalogos.tipo-cliente', compact('datas'));
    }
    ...
    public function save()
    {
        ...
        try {
            $this->validate();
            Ctg_Tipo_Cliente::create($this->only(['name']));
            DB::commit();
            $this->reset('name');
    ...
    #[Validate('required')]
    public $name;
    public function setData(Ctg_Tipo_Cliente $data)
    {
        $this->limpiar();
        $this->name = $data->name;
    ...
    public function update()
    {
        ...
        try {
            ...
            $data = Ctg_Tipo_Cliente::find($this->data_id);
            $data->update([
    ...
    #[On('delete-data')]
    public function delete(Ctg_Tipo_Cliente $data)
    {
        try {
            $data->delete();
    ...
    #[On('baja-data')]
    public function baja(Ctg_Tipo_Cliente $data)
    {

        try {
            $data->update([
                'status_ctg_tipo_cliente' => 0,
            ]);
            $this->dispatch('success-data', "El registro se dio de baja correctamente.");
        } catch (\Exception $e) {
            $this->dispatch('error', "No se pudo completar la accion, intente mas tarde.");
    ...
    #[On('delete-reactivar')]
    public function reactivar(Ctg_Tipo_Cliente $data)
    {
        try {
            $data->update([
                'status_ctg_tipo_cliente' => 1,
            ]);
