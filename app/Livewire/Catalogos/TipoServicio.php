...
class TipoServicio extends Component
{
    ...
    {
        if ($this->readyToLoad) {
            $datas =  CtgTipoServicio::where('name', 'ilike', '%' . $this->search . '%')
                ->orderBy('id', 'DESC')->paginate(10);;
        } else {
            $datas = [];
        }
        return view('livewire.catalogos.tipo-servicio', compact('datas'));
    }
    ...
    public function save()
    {
        ...
        try {
            $this->validate();
            CtgTipoServicio::create($this->only(['name']));
            DB::commit();
            $this->reset('name');
    ...
    #[Validate('required')]
    public $name;
    public function setData(CtgTipoServicio $data)
    {
        $this->limpiar();
        $this->name = $data->name;
    ...
    public function update()
    {
        ...
        try {
            ...
            $data = CtgTipoServicio::find($this->data_id);
            $data->update([
    ...
    #[On('delete-data')]
    public function delete(CtgTipoServicio $data)
    {
        try {
            $data->delete();
    ...
    #[On('baja-data')]
    public function baja(CtgTipoServicio $data)
    {

        try {
            $data->update([
                'status_ctg_tipo_servicio' => 0,
            ]);
            $this->dispatch('success-data', "El registro se dio de baja correctamente.");
        } catch (\Exception $e) {
            $this->dispatch('error', "No se pudo completar la accion, intente mas tarde.");
    ...
    #[On('delete-reactivar')]
    public function reactivar(CtgTipoServicio $data)
    {
        try {
            $data->update([
                'status_ctg_tipo_servicio' => 1,
            ]);
